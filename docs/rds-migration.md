...
|--------|---------|---------|
| `auth:migrate` | `bunx better-auth migrate --yes --config ./src/auth/index.ts` | Better Auth tables |
| `db:apply` | `bun run src/scripts/apply-sql-migrations.ts` | Idempotent additive SQL in `backend/drizzle/*.sql` |
