import { describe, expect, test } from "bun:test";
import { imageFileToPdf } from "./image-to-pdf";

const PNG_1X1 = Uint8Array.from(
  atob("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg=="),
  (c) => c.charCodeAt(0)
);

// Minimal valid JPEG (1x1) bytes
const JPEG_1X1 = Uint8Array.from([
  ,
]);

describe("imageFileToPdf", () => {
  test("converts PNG to PDF", async () => {
    const file = new File([PNG_1X1], "photo.png", { type: "image/png" });
    const pdf = await imageFileToPdf(file);
    expect(pdf.type).toBe("application/pdf");
    expect(pdf.name).toBe("photo.pdf");
    const header = new Uint8Array(await pdf.slice(0, 5).arrayBuffer());
    expect(String.fromCharCode(...header)).toBe("%PDF-");
  });

  test("converts JPEG to PDF", async () => {
    const file = new File([JPEG_1X1], "photo.jpg", { type: "image/jpeg" });
    const pdf = await imageFileToPdf(file);
    expect(pdf.type).toBe("application/pdf");
    const header = new Uint8Array(await pdf.slice(0, 5).arrayBuffer());
    expect(String.fromCharCode(...header)).toBe("%PDF-");
  });

  test("rejects unsupported image types", async () => {
    const file = new File([new Uint8Array([1, 2, 3])], "anim.gif", { type: "image/gif" });
    await expect(imageFileToPdf(file)).rejects.toThrow("Photo must be JPEG or PNG.");
  });
});
